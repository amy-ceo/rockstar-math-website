...
exports.addPurchasedClass = async (req, res) => {
  try {
    ...
    if (isCommonCorePurchased) {
      ...
      const commonCoreZoomLink =
        'https://us06web.zoom.us/meeting/register/mZHoQiy9SqqHx69f4dejgg#/registration' // ✅ Change to actual link
    ...
    if (zoomCoursesPurchased.length > 0 || servicePurchased.length > 0) {
      ...
      if (zoomCoursesPurchased.length > 0) {
        emailHtml += `<h3>🔗 Here are your Zoom links:</h3><ul>${zoomCoursesPurchased
          .map(
            (name) =>
              `<li><a href="${ZOOM_LINKS[name]}" target="_blank">${ZOOM_LINKS[name]}</a></li>`,
          )
          .join('')}</ul>`
      }
