...
exports.captureOrder = async (req, res) => {
  try {
    ...
    // ✅ Step 2: Clear User's Cart from Database
    try {
      console.log('🛒 Clearing User Cart in Database...')
      await Register.findByIdAndUpdate(user._id, { $set: { cart: [] } }) // ✅ Empty user's cart
      console.log('✅ User Cart Cleared!')
    } catch (cartClearError) {
      console.error('❌ Error Clearing User Cart:', cartClearError)
    }
    ...
    res.json({
      ...
      clearCart: true,
      payment: captureResponse.result, // ✅ Yeh bhi include kar diya
    })
  } catch (error) {
