...
exports.captureOrder = async (req, res) => {
  try {
    ...
    const amount = captureResponse.result.purchase_units[0].payments?.captures?.[0]?.amount.value;
    ...
    try {
      ...
      return res.status(500).json({ error: "Failed to save payment.", details: saveError.message });
    }
