...
const CheckoutPage = () => {
  ...
  useEffect(() => {
    ...
    if (!storedCart || storedCart.length === 0) {
      ...
      const calculatedSubtotal = storedCart.reduce((total, item) => total + Number(item.price || 0), 0)
...
try {
      ...
      const response = await fetch('https://backend-production.example.com/api/stripe/create-payment-intent', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          amount: total,
  ...
  const handlePaymentSuccess = async () => {
    try {
      ...
      // ✅ Only clear the cart when payment is successfully captured
      clearCart();
      toast.success('🎉 Payment Successful! Redirecting...');
      navigate('/dashboard');
    } catch (error) {
          ...
          {!showPaymentForm && (
            <>
              ...
              <button
                onClick={async () => {
                  if (total > 0) {
                    const clientSecret = await createPaymentIntent() // ✅ Wait for Payment Intent
                    if (clientSecret) {
