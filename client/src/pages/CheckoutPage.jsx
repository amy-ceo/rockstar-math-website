...
const handlePaymentSuccess = async () => {
  try {
    ...
    const response = await fetch("http://localhost:5000/api/add-purchased-class", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        userId: user._id, // ✅ Fix applied
      ...
      <PayPalButtons
        ...
        createOrder={async () => {
          try {
            ...
            const response = await fetch("http://localhost:5000/api/paypal/create-order", { // ✅ Fix applied: Correct route
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ 
                userId: user._id, 
