...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchPurchasedClasses = async () => {
      setLoading(true)
      try {
        const response = await fetch(`https://backend-production-4b2c.up.railway.app/api/${users._id}/purchased-classes`)
    ...
    const fetchZoomMeeting = async () => {
      try {
        const response = await fetch(`https://backend-production-4b2c.up.railway.app/api/${users._id}/zoom-meeting`)
        const data = await response.json()
