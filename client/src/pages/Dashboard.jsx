...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchPurchasedClasses = async () => {
      setLoading(true)
      try {
        const response = await fetch(`http://localhost:5000/api/${users._id}/purchased-classes`)
    ...
    const fetchZoomMeeting = async () => {
      try {
        const response = await fetch(`http://localhost:5000/api/${users._id}/zoom-meeting`)
        const data = await response.json()
