...
const AdminNewsLetter = () => {
  ...
  const fetchSubscribers = async () => {
    try {
      const response = await axios.get(
        'https://backend-production-cbe2.up.railway.app/api/subscribers',
      )
  ...
  const updateSubscriber = async () => {
    try {
      await axios.put(`https://backend-production-cbe2.up.railway.app/api/subscribers/${editId}`, {
        email: editEmail,
      })
      setSubscribers(
        subscribers.map((sub) => (sub._id === editId ? { ...sub, email: editEmail } : sub)),
      )
      setEditId(null)
      setEditEmail('')
    } catch (error) {
                      ...
                      {editId === sub._id ? (
                        ...
                        <>
                          <button
                            ...
                            onClick={() => editSubscriber(sub._id, sub.email)}
                          >
                            <FaEdit />
                          </button>
                          <button
